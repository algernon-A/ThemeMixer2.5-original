    ...
    public class UltimateEyeCandyPatch
    {
        ...
        public void Patch(Harmony harmonyInstance)
        {
            try
            {
                ...
                {
                    Debug.LogError($"Failed to patch {nameof(UltimateEyeCandyPatch)}: target method or postfix method is null.");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Failed to patch {nameof(UltimateEyeCandyPatch)}: {ex}");
            }
        }
        ...
        public void Unpatch(Harmony harmonyInstance)
        {
            try
            {
                ...
                {
                    Debug.LogError($"Failed to unpatch {nameof(UltimateEyeCandyPatch)}: target method or postfix method is null.");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Failed to unpatch {nameof(UltimateEyeCandyPatch)}: {ex}");
            }
        }
        ...
        private static void Postfix(ref ColorManagementPanel __instance)
        {
            ...
            {
                Debug.LogError($"Failed to add event handler: {ex}");
            }
        }
        ...
        private static void OnLoadLutButtonClicked(ColossalFramework.UI.UIComponent component, ColossalFramework.UI.UIMouseEventParameter eventParam)
        {
            ...
            {
                Debug.LogError($"Failed to send message: {ex}");
            }
        }
